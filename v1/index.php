...
$app->post('/register', function() use ($app) {
            ...
            if ($res == USER_CREATED_SUCCESSFULLY) {
                ...
                $response["error"] = true;
                $response["errorCode"]="USER_ALREADY_EXISTED";
...
$app->post('/login', function() use ($app) {
	...
	$app->get('/products/:id', function($productId) use ($app) {
		$response = array();
		$db = new DbHandler();
	 ...
	 */
		if ($result != NULL) {
			...
			$response["productDescription"] = $result["productDescription"];
			$response["defaultQuantity"] = $result["defaultQuantity"];
			$response["quantity1"] = $result["quantity1"];
			$response["quantity2"] = $result["quantity2"];
			$response["quantity3"] = $result["quantity3"];
			$response["currentRate"] = $result["currentRate"];
			$response["category"] = $result["category"];
			$response["saleUnitLabel"] = $result["saleUnitLabel"];
			echoRespnse(200, $response);
		} else {
			$response["error"] = true;
	...
	$app->get('/products', function() use ($app) {
		...
		$app->get('/categories', function() use ($app) {
			...
			while ($task = $result->fetch_assoc()) {
				...
				$tmp["categoryName"] = $task["categoryName"];
				$tmp["categoryDescription"] = $task["categoryDescription"];
